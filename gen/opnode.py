...
from z3 import And, Not, Select, IntVal, is_const  # noqa: E402

from mobile.gen import concretize as _concretize  # noqa: E402
from mobile.engine.model import (  # noqa: E402
    _ARG_BUILDERS, TensorVar, TensorListVar, OptVar, IntArrayVar,
    MAX_DIM, MAX_INT_ARRAY_LEN,
)
from mobile.engine.op_lookup import find_op_for_symbol  # noqa: E402
from mobile.engine.verifier import _make_solver, _add_domain_axioms, _flatten  # noqa: E402
from mobile.engine.test_valid_inputs import _diverse_model  # noqa: E402

from mobile.gen.blocklist import is_blocked  # noqa: E402
from mobile.gen.executorch_allowlist import (  # noqa: E402
    is_executorch_op,
)

# opnode.py lives at mobile/gen/; approved_constraints/ is vendored INSIDE the mobile
# package (mobile/approved_constraints/) so the package references only its own content.
APPROVED = Path(__file__).resolve().parents[1] / "approved_constraints"
...
class OpNode:
        ...
        for var in self.named_vars.values():
            if isinstance(var, TensorVar):
                ...
                for i in range(MAX_INT_ARRAY_LEN):
                    ...
                    solver.add(cell >= -_MAX_NDIM, cell <= _MAX_SIZE)
    ...
    def generate(self, rng, pins=None):
        ...
        s = self._solver(pins)
        m = _diverse_model(s, self.named_vars, rng, None)
        if m is None:
            return None
        c = _build_call_args(
