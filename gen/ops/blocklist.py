...
# ── Complex-only ops (no real-dtype path; backends lack complex) ─────────────────
# Ops with NO real-dtype semantics: _conj_physical's portable kernel dispatches
# complex-only, and view_as_real_copy requires a complex input by definition. The
# generator collapses every complex dtype to float32 (gen/concretize/dtypes.py) and most
# backends can't represent complex at all (XNNPACK serializes 0 complex types; portable
# implements complex in only ~10 of 172 kernels; Vulkan none). So on the real inputs we
# feed, these can only SKIP ("Unhandled dtype") or fail the eager oracle — never a real
# finding. _conj_physical alone is the #1 portable SKIP (10,023 ≈ 27% of all SKIPs).
# See findings/portable/skip/06-complex-dtype-support.md.
_COMPLEX_ONLY_PREFIXES: tuple[str, ...] = (
    "_conj_physical",     # complex-only kernel → "Unhandled dtype" on reals; eager is real-identity
    "view_as_real_copy",  # requires a complex input → eager raises on reals
)
