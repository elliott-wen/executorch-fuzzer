#!/usr/bin/env python3
"""Repro: Vulkan MOTO-unique silent data corruption on `alias_copy` (CORRECT on ASUS & Samsung).
Corpus job w0:186; lowered to Vulkan, run on the connected Moto phone (broker 15554).
Moto returns [-3,-3,-3,-3,...] where eager/ASUS/Samsung give [-3,-3,-3,4,-3,-1,-4,-2]."""
...
fr=P.encode_pushjob("repro::alias_copy",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<90:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"alias_copy -> {s} {d}");print("  eager :",job.eager[0].flatten()[:8].tolist());print("  device:",et[0].flatten()[:8].tolist());break
