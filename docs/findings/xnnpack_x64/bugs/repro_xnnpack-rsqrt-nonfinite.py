#!/usr/bin/env python3
"""Repro: XNNPACK delegated bug `rsqrt` on a non-finite/boundary input (x86 host, xnnpack_client).
rsqrt(0.0) returns NaN on XNNPACK where ATen returns inf (fast-rsqrt has no zero-domain guard).
...
SRC="import torch\ndef _f(r):\n return r[0] if isinstance(r,(tuple,list)) else r\nL0=torch.tensor([0.0,4.0])\ndef g(L0):\n return (_f(torch.ops.aten.rsqrt.default(L0)),)\nLEAVES=[L0]\n"
...
fr=P.encode_pushjob("repro::rsqrt",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<60:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"rsqrt -> {s} {d}");print("  eager :",job.eager[0].tolist());print("  device:",et[0].tolist());break
