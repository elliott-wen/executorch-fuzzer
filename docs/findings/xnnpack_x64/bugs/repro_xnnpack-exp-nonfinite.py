#!/usr/bin/env python3
"""Repro: XNNPACK delegated bug `exp` on a non-finite/boundary input (x86 host, xnnpack_client).
exp(NaN) returns 0.0 on XNNPACK where ATen returns NaN (rational-poly exp, no NaN guard).
...
SRC="import torch\ndef _f(r):\n return r[0] if isinstance(r,(tuple,list)) else r\nL0=torch.tensor([float('nan'),1.0])\ndef g(L0):\n return (_f(torch.ops.aten.exp.default(L0)),)\nLEAVES=[L0]\n"
...
fr=P.encode_pushjob("repro::exp",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<60:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"exp -> {s} {d}");print("  eager :",job.eager[0].tolist());print("  device:",et[0].tolist());break
