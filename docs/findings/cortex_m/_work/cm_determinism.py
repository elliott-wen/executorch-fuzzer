#!/usr/bin/env python3
"""Determinism gate (analysis_single.md step 3b) for cortex-m: re-run each job_id N times on
the SAME .pte and tally statuses. all-N repro -> bug; >=1/N -> INTERMITTENT; 0/N -> drop.
Reuses the graph's own inputs (debug-mode `mobile feed <job_id>`) against the live cortex-m
broker on 15664/15666. Usage: cm_determinism.py N job_id [job_id ...]
"""
...
def one_pass(jid):
    p = subprocess.run(
        ["python", "-m", "mobile", "feed", jid, "--corpus", CORPUS, "--host", "127.0.0.1",
         "--job-port", "15664", "--ctrl-port", "15666", "--timeout", "300"],
