#!/usr/bin/env python3
"""Repro: Vulkan delegated bug `mean` on the ASUS phone. mean overflow saturates to fp16-max (65504) where ATen returns inf.
Corpus job w10:191; lowered to Vulkan, run on the connected phone (broker 15554)."""
...
fr=P.encode_pushjob(f"repro::mean",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<90:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"mean -> {s} {d}");print("  eager :",job.eager[0].flatten()[:6].tolist());print("  device:",et[0].flatten()[:6].tolist());break
