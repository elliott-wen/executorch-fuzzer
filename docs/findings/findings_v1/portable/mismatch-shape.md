...
> **AUDIT CORRECTION (2026-08-23).** The prose below attributes the divergence to the generator
> *reusing/aliasing* a node's preallocated `out=` buffer. Measured per case, the **aliasing is not
> the load-bearing ingredient**: perturbing the declared `out=` shape leaves the eager divergent
> shape unchanged in **484 of 485** cases, but de-aliasing the buffer (`nK` -> `nK.clone()`) repairs
> only **121 of 485** — for the other 358 the aliasing is irrelevant. The necessary ingredient is
> simply a **wrong-shaped preallocated `out=` buffer**; eager resizes it in place and static planning
> cannot. Method + raw data: `findings/MULTIOP_SMALL_CLAIMS_AUDIT.md` (claim D) and
> `findings/multiop_small_claims_probes/`.
>
> Consequence for anyone using this as a confound check: it must cover the **value** class as well as
> the shape class. XNNPACK's 2 measured GRAPHOPT rows and Ethos-U's `w78:202` are exactly that.
