#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_xor.Tensor_out` on ASUS a12201 (Vulkan).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:386 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([1, 4], dtype=torch.uint8).reshape([1, 1, 1, 2])\nB1 = torch.tensor([7, 7, 7, 7, 7, 7, 0, 5, 6, 6, 1, 1], dtype=torch.int64).reshape([3, 2, 2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.bitwise_xor.Tensor_out(B0, B1, out=torch.empty((1, 3, 2, 2), dtype=torch.float32))))\n    return (T,)\n'
