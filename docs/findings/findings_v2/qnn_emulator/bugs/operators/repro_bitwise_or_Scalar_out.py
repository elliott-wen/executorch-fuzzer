#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_or.Scalar_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:498 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([1], dtype=torch.uint8).reshape([])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.bitwise_or.Scalar_out(B0, -8, out=torch.empty((), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::bitwise_or_Scalar_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `bitwise_or.Scalar_out`  ->", status, detail)
