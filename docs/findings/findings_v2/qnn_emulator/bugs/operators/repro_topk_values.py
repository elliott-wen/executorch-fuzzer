#!/usr/bin/env python3
"""Repro: Vulkan operator bug `topk.values` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w112:127 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([1.1103515625, -0.482421875, 0.93408203125, 1.1640625], dtype=torch.float16).reshape([4])\nB1 = torch.tensor([1, 0, 2, 3], dtype=torch.int64).reshape([4])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.topk.values(B0, 4, -1, False, False, values=B0, indices=B1)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::topk_values", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `topk.values`  ->", status, detail)
