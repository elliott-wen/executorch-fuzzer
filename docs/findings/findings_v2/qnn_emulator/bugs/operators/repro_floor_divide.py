#!/usr/bin/env python3
"""Repro: Vulkan operator bug `floor_divide` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:382 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([1.0, 1.0, 1.0, 1.0], dtype=torch.float32).reshape([2, 2])\nB1 = torch.tensor([1], dtype=torch.uint8).reshape([1])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.floor_divide.default(B0, B1)))\n    return (T,)\n'
