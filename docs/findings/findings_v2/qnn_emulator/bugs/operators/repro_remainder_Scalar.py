#!/usr/bin/env python3
"""Repro: Vulkan operator bug `remainder.Scalar` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:555 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-3, -3, 4, 4, -1, -1, -2, -2, -1, -1, -1, -1, 3, 3, -4, -4, -3, -3, -3, -3, 4, 4, 0, 0], dtype=torch.int64).reshape([3, 2, 2, 2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.remainder.Scalar(B0, -8)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::remainder_Scalar", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `remainder.Scalar`  ->", status, detail)
