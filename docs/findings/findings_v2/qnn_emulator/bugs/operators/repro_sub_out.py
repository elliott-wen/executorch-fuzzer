#!/usr/bin/env python3
"""Repro: Vulkan operator bug `sub.out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:283 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.381591796875, 0.751953125], dtype=torch.float16).reshape([2])\nB1 = torch.tensor([0.513671875], dtype=torch.float16).reshape([1])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.sub.out(B0, B1, alpha=-7.0, out=torch.empty((2,), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::sub_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `sub.out`  ->", status, detail)
