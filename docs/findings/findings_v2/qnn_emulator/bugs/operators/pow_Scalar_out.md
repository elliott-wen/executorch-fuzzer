# Vulkan operator bug — `pow.Scalar_out`
...
Bisected from corpus job `w0:379` `out[3]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=3.770e+09 (rtol=0.01 atol=0.001)`
- input dtypes: ['torch.int32']
- eager  (CPU): [5764801.0, 5764801.0, 5764801.0, 5764801.0, 5764801.0, 1622647808.0]
- device (GPU): [5764801.0, 5764801.0, 5764801.0, 5764801.0, 5764801.0, -2147483648.0]

## Reproduce
```
python findings_v2/qnn_emulator/bugs/operators/repro_pow_Scalar_out.py
```
