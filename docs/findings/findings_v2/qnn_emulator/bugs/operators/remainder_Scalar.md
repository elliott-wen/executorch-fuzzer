...
Bisected from corpus job `w0:555` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=8.000e+00 (rtol=0.0 atol=0.0)`
- input dtypes: ['torch.int64']
- eager  (CPU): [-3.0, -3.0, -4.0, -4.0, -1.0]
- device (GPU): [-3.0, -3.0, 4.0, 4.0, -1.0]

## Reproduce
```
python findings_v2/qnn_emulator/bugs/operators/repro_remainder_Scalar.py
```
