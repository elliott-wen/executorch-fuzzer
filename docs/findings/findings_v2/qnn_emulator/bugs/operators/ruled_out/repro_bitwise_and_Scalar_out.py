#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_and.Scalar_out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:340 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([False, False, True, True, False, False, True, True, False, False, True, True, False, False, True, True], dtype=torch.bool).reshape([4, 4])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.bitwise_and.Scalar_out(B0, -3, out=torch.empty((4, 4), dtype=torch.uint8))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::bitwise_and_Scalar_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `bitwise_and.Scalar_out`  ->", status, detail)
