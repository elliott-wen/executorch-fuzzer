# Vulkan operator bug — `bitwise_and.Tensor_out`
...
**Stability:** NON-DETERMINISTIC — wrong output on 1/7 reruns, correct on 3/7 of the SAME baked input (race / uninitialized memory)

## Evidence — single-op isolation with exact baked inputs
Bisected from corpus job `w101:730` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
- **worst element** (idx 5): eager **0.0** vs device **1.0**
- input dtypes: ['torch.bool', 'torch.bool']
- sample idx [0, 1, 2, 3, 4, 5] — eager  [1.0, 1.0, 1.0, 0.0, 1.0, 0.0]
- sample idx [0, 1, 2, 3, 4, 5] — device [1.0, 1.0, 1.0, 0.0, 1.0, 1.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_bitwise_and_Tensor_out.py
```
