# Vulkan operator bug — `remainder.Tensor_out`
...
Bisected from corpus job `w109:382` `out[1]`. The op was rebuilt as a **one-op graph fed its
...
- **worst element** (idx 2): eager **-2.0** vs device **2.0**
- input dtypes: ['torch.uint8', 'torch.int32']
- sample idx [0, 1, 2, 3, 4] — eager  [-1.0, 0.0, -2.0, -1.0, 0.0]
- sample idx [0, 1, 2, 3, 4] — device [2.0, 0.0, 2.0, 2.0, 0.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_remainder_Tensor_out.py
```
