...
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:163 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-3, 4, -3, -3, -3, -1], dtype=torch.int32).reshape([1, 3, 2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.bitwise_left_shift.Tensor_Scalar_out(B0, -3, out=torch.empty((1, 3, 2), dtype=torch.float32))))\n    return (T,)\n'
