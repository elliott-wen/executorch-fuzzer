# Vulkan operator bug — `index_select`
...
Bisected from corpus job `w108:245` `out[3]`. The op was rebuilt as a **one-op graph fed its
...
- **worst element** (idx 13): eager **4.0** vs device **1.0**
- input dtypes: ['torch.float32', 'torch.int32']
- sample idx [0, 1, 2, 3, 4, 13] — eager  [0.0, 1.0, 0.0, 2.0, 4.0, 4.0]
- sample idx [0, 1, 2, 3, 4, 13] — device [0.0, 1.0, 0.0, 2.0, 4.0, 1.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_index_select.py
```
