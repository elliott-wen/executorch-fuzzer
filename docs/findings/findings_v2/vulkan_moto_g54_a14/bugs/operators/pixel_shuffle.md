# Vulkan operator bug — `pixel_shuffle`
...
Bisected from corpus job `w101:415` `out[3]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=7.000e+00 (rtol=0.01 atol=0.001)`
- **worst element** (idx 15): eager **-4.0** vs device **3.0**
- input dtypes: ['torch.float32']
- sample idx [0, 1, 2, 3, 4, 15] — eager  [3.0, -0.0, 3.0, -0.0, -3.0, -4.0]
- sample idx [0, 1, 2, 3, 4, 15] — device [3.0, 3.0, 3.0, 3.0, -3.0, 3.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_pixel_shuffle.py
```
