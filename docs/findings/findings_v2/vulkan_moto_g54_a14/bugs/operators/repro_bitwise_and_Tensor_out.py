#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_and.Tensor_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w1:424 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([True, True], dtype=torch.bool).reshape([1, 2])\nB1 = torch.tensor([True, True, True, False, True, False, True, True], dtype=torch.bool).reshape([2, 2, 2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.bitwise_and.Tensor_out(B0, B1, out=torch.empty((2, 2, 2), dtype=torch.bool))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::bitwise_and_Tensor_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `bitwise_and.Tensor_out`  ->", status, detail)
