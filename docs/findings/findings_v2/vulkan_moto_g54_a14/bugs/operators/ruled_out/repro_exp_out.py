#!/usr/bin/env python3
"""Repro: Vulkan operator bug `exp.out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w103:526 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.8858417272567749, 0.4443448781967163, -0.18311353027820587, -0.18311353027820587, -0.2373933047056198, -0.2373933047056198], dtype=torch.float32).reshape([3, 2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.exp.out(B0, out=torch.empty((3, 2), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::exp_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `exp.out`  ->", status, detail)
