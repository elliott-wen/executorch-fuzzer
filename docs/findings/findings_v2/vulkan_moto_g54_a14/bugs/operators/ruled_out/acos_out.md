...
**Status:** not filed as an operator bug. reproduces eager when it runs (4/7 clean) — NOT a kernel bug; the single earlier GENUINE was near-tolerance/transient.

---

# Vulkan operator bug — `acos.out`
...
Bisected from corpus job `w12:591` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_acos_out.py
```
