...
**Status:** not filed as an operator bug. reproduces eager when it runs (5/7 clean) — NOT a kernel bug; the single earlier GENUINE was near-tolerance/transient.

---

# Vulkan operator bug — `atanh.out`
...
Bisected from corpus job `w118:686` `out[3]`. The op was rebuilt as a **one-op graph fed its
...
## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_atanh_out.py
```
