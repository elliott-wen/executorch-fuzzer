...
**Status:** not filed as an operator bug. reproduces eager when it runs (1/7 clean) — NOT a kernel bug; the single earlier GENUINE was near-tolerance/transient.

---

# Vulkan operator bug — `abs`
...
Bisected from corpus job `w100:731` `out[1]`. The op was rebuilt as a **one-op graph fed its
...
## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_abs.py
```
