#!/usr/bin/env python3
"""Repro: Vulkan operator bug `div.out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w102:518 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-3, -4, -4], dtype=torch.int32).reshape([3, 1])\nB1 = torch.tensor([7.0, 7.0, 6.0, 6.0, 11.0, 11.0, 7.0, 7.0, 6.0, 6.0, 11.0, 11.0, 7.0, 7.0, 6.0, 6.0, 11.0, 11.0], dtype=torch.float16).reshape([3, 3, 2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.div.out(B0, B1, out=torch.empty((3, 3, 2), dtype=torch.float32))))\n    return (T,)\n'
