#!/usr/bin/env python3
"""Repro: Vulkan operator bug `split_with_sizes_copy` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:620 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-3, -3, -3, 4, -3, 2, 1, 0, -3, -2, 4, 4, 4, -4, -1, -4], dtype=torch.int64).reshape([2, 4, 2, 1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.split_with_sizes_copy.default(B0, [1, 1], 2)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::split_with_sizes_copy", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `split_with_sizes_copy`  ->", status, detail)
