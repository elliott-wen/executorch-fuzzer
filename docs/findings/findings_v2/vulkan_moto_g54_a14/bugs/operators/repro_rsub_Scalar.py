#!/usr/bin/env python3
"""Repro: Vulkan operator bug `rsub.Scalar` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:705 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([4, 3, 1, 4, 0, 1], dtype=torch.uint8).reshape([3, 2, 1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.rsub.Scalar(B0, 5.0, 1.0)))\n    return (T,)\n'
