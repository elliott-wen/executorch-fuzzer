#!/usr/bin/env python3
"""Repro: Vulkan operator bug `transpose_copy.int` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:196 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0, 0, 0, 4, 0, 0, 0, 0, 0, 0, 4, 0, 4, 3, 0, 0, 0, 0, 1, 0, 2, 4, 1, 0, 1, 1, 0, 0, 0, 1, 2, 0, 0, 0, 2, 4], dtype=torch.int64).reshape([3, 4, 3])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.transpose_copy.int(B0, 0, 0)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::transpose_copy_int", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `transpose_copy.int`  ->", status, detail)
