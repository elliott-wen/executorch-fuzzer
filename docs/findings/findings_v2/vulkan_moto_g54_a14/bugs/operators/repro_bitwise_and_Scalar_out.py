#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_and.Scalar_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w10:574 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-1], dtype=torch.int16).reshape([])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.bitwise_and.Scalar_out(B0, 6, out=torch.empty((), dtype=torch.float32))))\n    return (T,)\n'
