#!/usr/bin/env python3
"""Repro: aten.index_put does not write its values on the Vulkan delegate (Moto G54).

Phone-verified behavior (accumulate=False): the device returns the input `self` with element 0
replaced by 0, and the indexed values are never written. accumulate=True returns `self`
unchanged. Single-op graph -> no aliasing.

Phone-verified cases (Moto G54, 2026-06-29):
  index_put([1,2,3,4],  [[1,3]], [7,8], False)  eager [1,7,3,8]   device [0,2,3,4]
  index_put([10,20,30,40], [[2]], [99], False)  eager [10,20,99,40] device [0,20,30,40]
  index_put([1,2,3,4],  [[1,3]], [7,8], True )  eager [1,9,3,12]  device [1,2,3,4]

Needs a connected Vulkan worker (broker on 127.0.0.1:15554).
"""
import os, sys, time, warnings, logging
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "")
warnings.filterwarnings("ignore"); logging.disable(logging.INFO)
sys.path.insert(0, "/data/jwen929")
import zmq
from mobile.gen.export import build_job
from mobile.executor import protocol as P, compare as cmp

CASES = {
 "index_put([1,2,3,4],[[1,3]],[7,8],accumulate=False)": """import torch
x = torch.tensor([1., 2., 3., 4.])
LEAVES = [x]
def g(x):
    idx = torch.tensor([1, 3], dtype=torch.int64); vals = torch.tensor([7., 8.])
    return (torch.ops.aten.index_put.default(x, [idx], vals, False),)
""",
 "index_put([1,2,3,4],[[1,3]],[7,8],accumulate=True)": """import torch
x = torch.tensor([1., 2., 3., 4.])
LEAVES = [x]
def g(x):
    idx = torch.tensor([1, 3], dtype=torch.int64); vals = torch.tensor([7., 8.])
    return (torch.ops.aten.index_put.default(x, [idx], vals, True),)
""",
}

def run(src, name):
    job = build_job(src, "vulkan", False)
    assert job.status == "READY", getattr(job, "detail", job.status)
    ctx = zmq.Context.instance(); d = ctx.socket(zmq.DEALER); d.set_hwm(8)
    d.setsockopt(zmq.HEARTBEAT_IVL, 5000); d.setsockopt(zmq.HEARTBEAT_TIMEOUT, 20000)
    d.connect("tcp://127.0.0.1:15554"); pol = zmq.Poller(); pol.register(d, zmq.POLLIN)
    frames = P.encode_pushjob(f"repro::{name}", job.pte, job.inputs, job.eager,
                              desc=name, user_pos=job.user_pos)
    d.send_multipart(P.job_frames_from_pushjob(frames))
    t0 = time.monotonic()
    while time.monotonic() - t0 < 40:
        if dict(pol.poll(1000)).get(d):
            _, st, det, outs = P.decode_result(d.recv_multipart())
            if st != "RAN":
                print(f"  device status={st} {det}"); return
            et = cmp.select(outs, job.user_pos)
            for a, b in zip(job.eager, et):
                eg = a.flatten().tolist(); dv = b.flatten().tolist()
                bad = "  <-- DIVERGES" if eg != dv else "  (ok)"
                print(f"  eager={eg}  device={dv}{bad}")
            return
    print("  TIMEOUT")

if __name__ == "__main__":
    for name, src in CASES.items():
        print(name); run(src, name)
