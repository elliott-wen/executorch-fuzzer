#!/usr/bin/env python3
"""Repro: Vulkan operator bug `roll` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:217 out[2]),
...
fr = P.encode_pushjob("repro::roll", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
