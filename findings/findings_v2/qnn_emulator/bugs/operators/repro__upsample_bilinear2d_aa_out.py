#!/usr/bin/env python3
"""Repro: Vulkan operator bug `_upsample_bilinear2d_aa.out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w14:86 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([4, 3, 4, 4, 3, 4, 3, 1], dtype=torch.uint8).reshape([1, 2, 1, 4])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten._upsample_bilinear2d_aa.out(B0, [1, 3], False, None, None, out=torch.empty((1, 2, 1, 3), dtype=torch.uint8))))\n    return (T,)\n'
