...
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w108:506 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-0.4824913740158081], dtype=torch.float32).reshape([1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten._log_softmax.out(B0, 0, False, out=torch.empty((1,), dtype=torch.float32))))\n    return (T,)\n'
