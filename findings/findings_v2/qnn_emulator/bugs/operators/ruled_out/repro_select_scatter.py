#!/usr/bin/env python3
"""Repro: Vulkan operator bug `select_scatter` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:147 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0, 1, 3, 4], dtype=torch.uint8).reshape([2, 2])\nB1 = torch.tensor([nan, nan], dtype=torch.float16).reshape([2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.select_scatter.default(B0, B1, 0, 1)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::select_scatter", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `select_scatter`  ->", status, detail)
