#!/usr/bin/env python3
"""Repro: Vulkan operator bug `neg.out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:63 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([1], dtype=torch.int64).reshape([1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.neg.out(B0, out=torch.empty((1,), dtype=torch.int64))))\n    return (T,)\n'
