#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_xor.Tensor_out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w106:238 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([7, 7, 7], dtype=torch.uint8).reshape([3])\nB1 = torch.tensor([7, 7, 4], dtype=torch.uint8).reshape([3])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.bitwise_xor.Tensor_out(B0, B1, out=torch.empty((3,), dtype=torch.uint8))))\n    return (T,)\n'
