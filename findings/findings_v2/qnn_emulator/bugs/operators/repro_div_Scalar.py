#!/usr/bin/env python3
"""Repro: Vulkan operator bug `div.Scalar` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w108:379 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([2], dtype=torch.int32).reshape([1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.div.Scalar(B0, 1)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::div_Scalar", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `div.Scalar`  ->", status, detail)
