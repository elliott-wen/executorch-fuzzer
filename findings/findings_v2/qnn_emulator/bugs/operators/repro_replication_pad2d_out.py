#!/usr/bin/env python3
"""Repro: Vulkan operator bug `replication_pad2d.out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:197 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-0.73095703125, -0.73095703125, -0.73095703125, 0.26904296875], dtype=torch.float16).reshape([2, 2, 1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.replication_pad2d.out(B0, [4, -2, 4, -3], out=torch.empty((2, 3, 3), dtype=torch.float16))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::replication_pad2d_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `replication_pad2d.out`  ->", status, detail)
