#!/usr/bin/env python3
"""Repro: Vulkan operator bug `prod.int_out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:62 out[3]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-2.072575807571411, 0.9006140828132629], dtype=torch.float32).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.prod.int_out(B0, 0, True, dtype=None, out=torch.empty((1,), dtype=torch.int16))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::prod_int_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `prod.int_out`  ->", status, detail)
