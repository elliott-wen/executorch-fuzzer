#!/usr/bin/env python3
"""Repro: Vulkan operator bug `index_select.out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w22:615 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, -1.0, -1.0, -4.0, -1.0, -4.0, -68.0, 1.0, -2.0, -3.0, -2.0, 12.0, 0.0, -2.0, 15.0, -1.0], dtype=torch.float32).reshape([2, 2, 4])\nB1 = torch.tensor([1, 1], dtype=torch.int32).reshape([2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.index_select.out(B0, 1, B1, out=torch.empty((2, 2, 4), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::index_select_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `index_select.out`  ->", status, detail)
