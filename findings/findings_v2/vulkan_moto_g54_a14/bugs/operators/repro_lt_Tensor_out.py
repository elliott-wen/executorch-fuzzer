#!/usr/bin/env python3
"""Repro: Vulkan operator bug `lt.Tensor_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w103:700 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-3, -3, -3, 4, -3, -1, -4, -2, -3, -1, 4, -1, 4, 3, -3, -4, -3, -3, 1, -3, 2, 4, 1, 0, 1, 1, -2, -2, 0, 1, 2, -1, -3, -4, 2, 4, -2, -4, -1, -1, 4, -3, -1, -4, 4, 1, -1, -2, 4, 2, 3, 0, -4, -3], dtype=torch.int64).reshape([3, 2, 3, 3])\nB1 = torch.tensor([-4, 2, 1], dtype=torch.int64).reshape([3])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.lt.Tensor_out(B0, B1, out=torch.empty((3, 2, 3, 3), dtype=torch.uint8))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::lt_Tensor_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `lt.Tensor_out`  ->", status, detail)
