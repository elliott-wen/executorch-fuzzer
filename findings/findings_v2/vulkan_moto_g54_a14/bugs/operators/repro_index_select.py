#!/usr/bin/env python3
"""Repro: Vulkan operator bug `index_select` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w108:245 out[3]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, 1.0, 0.0, 2.0, 4.0, 1.0, 4.0, 1.0, 3.0, 2.0, 4.0, 1.0, 0.0, 1.0, 2.0, 2.0, 0.0, 1.0, 0.0, 4.0, 0.0, 3.0, 4.0, 3.0, 0.0, 4.0, 3.0, 3.0, 4.0, 3.0, 0.0, 4.0, 2.0, 3.0, 0.0, 3.0, 0.0, 0.0, 0.0, 1.0, 4.0, 2.0, 0.0, 0.0, 0.0, 1.0, 1.0, 2.0, 0.0, 0.0, 2.0, 1.0, 0.0, 2.0], dtype=torch.float32).reshape([3, 3, 3, 2])\nB1 = torch.tensor([0, 0], dtype=torch.int32).reshape([2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.index_select.default(B0, 1, B1)))\n    return (T,)\n'
