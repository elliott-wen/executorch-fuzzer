#!/usr/bin/env python3
"""Repro: Vulkan operator bug `clone` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w102:507 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([False, False, True, True, False, False, True, False, False, True, False, True, False, True, True, True, True, False], dtype=torch.bool).reshape([2, 3, 3])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.clone.default(B0, memory_format=None)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::clone", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
