#!/usr/bin/env python3
"""Repro: Vulkan operator bug `gather.out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:346 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([1, 1], dtype=torch.int32).reshape([1, 2])\nB1 = torch.tensor([0, 0, 0, 0, 0, 0], dtype=torch.int32).reshape([3, 2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.gather.out(B0, -2, B1, sparse_grad=False, out=torch.empty((3, 2), dtype=torch.int32))))\n    return (T,)\n'
