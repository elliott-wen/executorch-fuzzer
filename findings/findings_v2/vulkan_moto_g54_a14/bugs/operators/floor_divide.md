...
Bisected from corpus job `w100:358` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=1.300e+01 (rtol=0.01 atol=0.001)`
- **worst element** (idx 48): eager **9.0** vs device **-4.0**
- input dtypes: ['torch.int8', 'torch.float32']
- sample idx [0, 1, 2, 3, 4, 48] — eager  [-4.0, -3.0, 2.0, 3.0, -4.0, 9.0]
- sample idx [0, 1, 2, 3, 4, 48] — device [-4.0, -3.0, 2.0, 3.0, -4.0, -4.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_floor_divide.py
```
