#!/usr/bin/env python3
"""Repro: Vulkan operator bug `any.all_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:283 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.7509765625, 0.98681640625], dtype=torch.float16).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.any.all_out(B0, out=torch.empty((), dtype=torch.uint8))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::any_all_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `any.all_out`  ->", status, detail)
