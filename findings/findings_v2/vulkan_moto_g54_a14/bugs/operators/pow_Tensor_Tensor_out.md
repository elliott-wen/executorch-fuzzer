...
Bisected from corpus job `w103:125` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=2.541e+03 (rtol=0.01 atol=0.001)`
- **worst element** (idx 0): eager **2541.0** vs device **0.0**
- input dtypes: ['torch.int8', 'torch.uint8']
- sample idx [0, 1, 2, 3, 4] — eager  [2541.0, 2541.0, 256.0, 256.0, -1.0]
- sample idx [0, 1, 2, 3, 4] — device [0.0, 0.0, 256.0, 256.0, -1.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_pow_Tensor_Tensor_out.py
```
