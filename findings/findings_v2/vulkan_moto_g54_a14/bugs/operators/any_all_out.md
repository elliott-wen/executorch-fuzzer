# Vulkan operator bug — `any.all_out`
...
**Stability:** HIGH — GENUINE on 2/7 re-verification reruns (earlier 3/3 flake was transient device noise)

## Evidence — single-op isolation with exact baked inputs
Bisected from corpus job `w101:283` `out[1]`. The op was rebuilt as a **one-op graph fed its
...
- **worst element** (idx 0): eager **0.0** vs device **1.0**
...
## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_any_all_out.py
```
