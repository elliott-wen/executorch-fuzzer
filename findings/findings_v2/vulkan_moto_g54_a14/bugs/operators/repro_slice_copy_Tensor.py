#!/usr/bin/env python3
"""Repro: Vulkan operator bug `slice_copy.Tensor` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:336 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, 0.0, -0.4824913740158081, 1.1635862588882446, 2.0443692207336426, 0.0], dtype=torch.float32).reshape([2, 3, 1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.slice_copy.Tensor(B0, 1, None, None, 3)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::slice_copy_Tensor", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `slice_copy.Tensor`  ->", status, detail)
