#!/usr/bin/env python3
"""Repro: Vulkan operator bug `gt.Tensor_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w102:284 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0, 0, 0, 0, 0, 0, 0, 0], dtype=torch.uint8).reshape([2, 4])\nB1 = torch.tensor([-3], dtype=torch.int64).reshape([])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.gt.Tensor_out(B0, B1, out=torch.empty((2, 4), dtype=torch.uint8))))\n    return (T,)\n'
