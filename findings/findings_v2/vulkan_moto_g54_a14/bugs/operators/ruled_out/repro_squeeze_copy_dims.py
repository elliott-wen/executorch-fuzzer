#!/usr/bin/env python3
"""Repro: Vulkan operator bug `squeeze_copy.dims` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:613 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-9.482491493225098, -6.955630779266357], dtype=torch.float32).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.squeeze_copy.dims(B0, [])))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::squeeze_copy_dims", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `squeeze_copy.dims`  ->", status, detail)
