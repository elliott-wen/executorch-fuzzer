...
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:1068 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.482421875, 2.044921875], dtype=torch.float16).reshape([2])\nB1 = torch.tensor([0], dtype=torch.int64).reshape([])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.index_select.out(B0, -1, B1, out=torch.empty((1,), dtype=torch.float16))))\n    return (T,)\n'
