#!/usr/bin/env python3
"""Repro: Vulkan operator bug `pow.Scalar_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w116:575 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([4, 4], dtype=torch.int32).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.pow.Scalar_out(7.0, B0, out=torch.empty((2,), dtype=torch.float32))))\n    return (T,)\n'
