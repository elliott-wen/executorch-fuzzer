#!/usr/bin/env python3
"""Repro: Vulkan operator bug `masked_fill.Scalar` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w115:137 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-3, -3, -3, 4, -3, -1, -4, -2, -3, -1, 4, -1, -3, -3, -3, 4, -3, -1, -4, -2, -3, -1, 4, -1], dtype=torch.int32).reshape([2, 2, 3, 2])\nB1 = torch.tensor([False], dtype=torch.bool).reshape([])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.masked_fill.Scalar(B0, B1, 0)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::masked_fill_Scalar", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `masked_fill.Scalar`  ->", status, detail)
