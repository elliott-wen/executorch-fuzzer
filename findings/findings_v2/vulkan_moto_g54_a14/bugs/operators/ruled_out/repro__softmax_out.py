#!/usr/bin/env python3
"""Repro: Vulkan operator bug `_softmax.out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w108:765 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-0.4824913740158081, 2.0443692207336426], dtype=torch.float32).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten._softmax.out(B0, 0, False, out=torch.empty((2,), dtype=torch.float32))))\n    return (T,)\n'
