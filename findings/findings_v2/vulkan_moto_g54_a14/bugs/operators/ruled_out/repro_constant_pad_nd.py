#!/usr/bin/env python3
"""Repro: Vulkan operator bug `constant_pad_nd` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:516 out[0]),
...
fr = P.encode_pushjob("repro::constant_pad_nd", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `constant_pad_nd`  ->", status, detail)
