#!/usr/bin/env python3
"""Repro: Vulkan operator bug `gt.Tensor_out` on ASUS a12201 (Vulkan).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:127 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-2147483648], dtype=torch.int32).reshape([1, 1])\nB1 = torch.tensor([inf, 1.0], dtype=torch.float32).reshape([2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.gt.Tensor_out(B0, B1, out=torch.empty((1, 2), dtype=torch.uint8))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::gt_Tensor_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `gt.Tensor_out`  ->", status, detail)
