#!/usr/bin/env python3
"""Repro: Vulkan operator bug `atanh.out` on ASUS a12201 (Vulkan).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w11:622 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, 0.0, 0.76171875, 0.0], dtype=torch.float16).reshape([4])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.atanh.out(B0, out=torch.empty((4,), dtype=torch.float16))))\n    return (T,)\n'
