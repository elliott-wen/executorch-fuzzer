#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_right_shift.Tensor_Scalar` on ASUS a12201 (Vulkan).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w106:379 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([9, 9, 9, 16, 1, 1, 1, 8, 13, 13, 13, 20, 5, 5, 5, 12], dtype=torch.int32).reshape([2, 2, 4])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.bitwise_right_shift.Tensor_Scalar(B0, -8)))\n    return (T,)\n'
