#!/usr/bin/env python3
"""Repro: Vulkan delegated bug `full` on the ASUS phone. full(1.0) returns 0.0 — constant-fill reads the wrong buffer.
Corpus job w110:446; lowered to Vulkan, run on the connected phone (broker 15554)."""
...
fr=P.encode_pushjob(f"repro::full",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<90:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"full -> {s} {d}");print("  eager :",job.eager[0].flatten()[:6].tolist());print("  device:",et[0].flatten()[:6].tolist());break
