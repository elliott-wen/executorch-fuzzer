#!/usr/bin/env python3
"""Repro: Vulkan delegated bug `copy` on the ASUS phone. copy returns a DIFFERENT buffer (copy-elision/aliasing) — output unrelated to input.
Corpus job w0:264; lowered to Vulkan, run on the connected phone (broker 15554)."""
...
fr=P.encode_pushjob(f"repro::copy",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<90:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"copy -> {s} {d}");print("  eager :",job.eager[0].flatten()[:6].tolist());print("  device:",et[0].flatten()[:6].tolist());break
