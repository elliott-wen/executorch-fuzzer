#!/usr/bin/env python3
"""Repro: XNNPACK delegated bug `maximum` on a non-finite/boundary input (x86 host, xnnpack_client).
maximum(NaN,finite) returns the finite operand on XNNPACK (SIMD fmax launders NaN) where ATen returns NaN.
...
fr=P.encode_pushjob("repro::maximum",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<60:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"maximum -> {s} {d}");print("  eager :",job.eager[0].tolist());print("  device:",et[0].tolist());break
