#!/usr/bin/env python3
"""Repro: QNN HTP rsub.Scalar wrong value (18 -> 9). Corpus job w0:1191.
Needs a QNN emulator worker on the broker (BROKER_JOB_PORT, default 15564)."""
...
fr=P.encode_pushjob("repro::rsub",job.pte,job.inputs,job.eager,desc="repro",user_pos=job.user_pos,delegated={"ops":1})
...
while time.monotonic()-t0<150:
    if dict(PL.poll(1000)).get(D):
        ...
        print(f"rsub -> {s} {d}");print("  eager :",job.eager[0].flatten()[:6].tolist());print("  device:",et[0].flatten()[:6].tolist());break
