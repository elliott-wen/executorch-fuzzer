#!/usr/bin/env python3
"""CRASH REPLAY: unfold_copy (and dim-ops) on a 0-D scalar input.

Reproduces a native SIGABRT (exit 134) from the portable `unfold_copy` validator
calling `self.size(0)` on a 0-D tensor, firing a fatal ET_CHECK in
`tensor_impl.h:136` ("... range [0, -1], got 0").

Usage: python findings/portable/crash/replay_02-unfold_copy-zero-dim.py
"""
import os
import subprocess
import sys
...
JOB = "corpus/portable/w0/w0_104.job"
