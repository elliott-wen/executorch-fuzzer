...
def _load_quantized_kernels() -> bool:
    """Register the quantized_decomposed::{quantize,dequantize}_per_{tensor,channel} kernels.

    A --quantize corpus needs them: PT2E leaves q/dq nodes OUTSIDE the delegate wherever the
    partitioner doesn't absorb them, and the pip `_portable_lib` runtime ships WITHOUT those
    kernels — so such a .pte fails to LOAD with "Missing operator:
    quantized_decomposed::quantize_per_tensor.out" (0x14). That surfaces as a SKIP, which is
    indistinguishable from a real backend coverage gap and would silently poison a float-vs-int8
    comparison. dlopening the wheel's aot lib RTLD_GLOBAL registers them into the same registry.
    No-op (and harmless) for a float corpus.
    """
    try:
        import ctypes
        import executorch.kernels.quantized as _q
        lib = os.path.join(os.path.dirname(_q.__file__), "libquantized_ops_aot_lib.so")
