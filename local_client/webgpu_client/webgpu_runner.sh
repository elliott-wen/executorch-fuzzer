#!/usr/bin/env bash
# webgpu_runner.sh — run ONE .pte carrying ExecuTorch WEBGPU-delegated subgraphs on the host,
# and emit its outputs. The Vulkan analog of vgf_runner.sh / cadence_runner.sh.
#
#   in:  --pte PTE  --input IN0 [--input IN1 ...]  --out OUTDIR  [--runner WEBGPU_RUNNER_BIN]
...
# Build the runner ONCE: mobile/local_client/webgpu_client/build_runner.sh.
set -uo pipefail
...
if [[ -z "$RUNNER" ]]; then
  RUNNER="${WEBGPU_RUNNER_BIN:-$MOBILE/tmp/webgpu_runner_build/webgpu_runner_io}"
fi
if [[ ! -x "$RUNNER" ]]; then
  echo "FATAL: webgpu_runner_io not found at '$RUNNER'. Build it: mobile/local_client/webgpu_client/build_runner.sh" >&2
  exit 3
fi

# --- Vulkan runtime env: just an ICD ------------------------------------------------------
# Pin lavapipe (CPU software Vulkan) so the run is headless and deterministic and does not
# probe the real GPUs on this box. Override with WEBGPU_VK_ICD to run on actual hardware.
VK_ICD="${WEBGPU_VK_ICD:-/usr/share/vulkan/icd.d/lvp_icd.x86_64.json}"
if [[ ! -r "$VK_ICD" ]]; then
  echo "FATAL: no Vulkan ICD at '$VK_ICD' (set WEBGPU_VK_ICD)" >&2
  exit 3
fi
...
# webgpu_runner_io writes OUTDIR/<4-wide-zero-padded>.bin (000i.bin). Normalize to out_<i>.bin.
shopt -s nullglob
n=0
