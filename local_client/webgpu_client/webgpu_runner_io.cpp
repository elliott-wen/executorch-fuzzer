/*
 * webgpu_runner_io.cpp — host x86 file-I/O runner for the ExecuTorch WebGPU delegate.
 *
 * IDENTICAL CODE to vulkan_runner_io.cpp on purpose. The .pte is the same file: WebGPU
 * lowering emits Vulkan's blob and registers under the id "VulkanBackend" (upstream's
 * test_lowers_identically_with_vulkan_backend_id asserts it, and WebGPUBackend.cpp does
 * Backend{"VulkanBackend", &cls}). The ONLY difference is the LINK LINE: this runner links
 * webgpu_backend where the other links vulkan_backend. They are mutually exclusive in one
 * binary because both claim that id — which is exactly why this is a separate runner.
 *
 ...
 * delegate targets core Vulkan compute rather than the ML extensions. webgpu_runner.sh
