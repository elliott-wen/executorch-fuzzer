"""vulkan_client.py — ExecuTorch VULKAN EXECUTOR worker (sibling of vgf_client.py).
...
`vulkan_runner.sh`, which invokes `vulkan_runner_io` — the ExecuTorch runtime linked with the
**Vulkan delegate** (vulkan_backend) + portable CPU kernels. Its subgraphs build compute
...
by vulkan_client/build_runner.sh, which is why this client shells out to a runner binary
...
— upstream's own test_vulkan_delegate.py marks those cases @disable_test. They arrive here as
load/dispatch failures, which the seam maps to SKIP rather than to a numeric divergence.

Usage:
    python vulkan_client/vulkan_client.py --host <broker-ip>
    python vulkan_client/vulkan_client.py --selftest    # plumbing test, no runner/Vulkan needed
"""
...
VULKAN_RUNNER = HERE / "vulkan_runner.sh"


def _rebuild_outputs(work: Path, out_metas: list, user_pos: list | None) -> list:
    """Rebuild the runner's output tensors. vulkan_runner.sh wrote each .pte output as raw
...
class VulkanExecutor:
    ...
    def run(self, job_id: str, pte: bytes, inputs: list, timeout: float,
        ...
        with tempfile.TemporaryDirectory(prefix="vulkan_job_") as d:
            work = Path(d)
            pte_path = work / "model.pte"
            pte_path.write_bytes(pte)
            cmd = [str(VULKAN_RUNNER), "--pte", str(pte_path), "--out", str(work),
                   "--target", self.target]
            ...
            if proc.returncode != 0:
                ...
                return P.encode_result(job_id, status, f"vulkan_runner rc={proc.returncode}: {tail}", None)
...
def run_client(host: str, client_port: int, ctrl_port: int, executor: VulkanExecutor,
    ...
    print(f"  vulkan_client[{label}] → broker tcp://{host}:{client_port} (target={executor.target})",
          flush=True)
    ...
    try:
        while True:
            ...
            ran += 1
            if ran % 50 == 0:
                print(f"  vulkan_client[{label}] ran {ran} jobs", flush=True)
    finally:
        executor.close()
        req.close(linger=0)
        sub.close(linger=0)
    print(f"  vulkan_client[{label}] done — ran {ran} jobs", flush=True)
    return 0
...
def main() -> int:
    ...
    ap.add_argument("--label", default="vulkan")
    ...
    ap.add_argument("--runner", default=None,
                    help="prebuilt vulkan_runner_io binary (else vulkan_runner.sh default)")
