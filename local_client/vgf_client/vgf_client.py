"""vgf_client.py — Arm VGF EXECUTOR worker (the Vulkan/VKML analog of cadence_client.py).

Same role as the Android client / fvp_client / nxp_client / cadence_client: pull `.pte` jobs
from the broker, run them, return the RAW output tensors over the language-neutral binary
protocol. The feeder holds the eager reference and does the diff — this process is a thin
executor.

The ONLY thing that differs from cadence_client.py is *where* the `.pte` runs: via
`vgf_runner.sh`, which invokes `vgf_runner_io` — the ExecuTorch runtime linked with the Arm
**VGF delegate** (vgf_backend) + portable CPU kernels. The VGF-delegated subgraphs JIT-compile
onto a Vulkan >= 1.3 device (here Mesa **lavapipe** CPU-software Vulkan + the ML SDK
**emulation layer**), so they execute on the host with NO NPU / GPU. VGF is FP-by-default
(QuantMode.OPTIONAL): float `.pte`s diff against the fp32 eager oracle; a quantized `.pte`
diffs against the stored quantized reference (a feeder/compare concern, not this client's).

Usage:
    python vgf_client/vgf_client.py --host <broker-ip>
    python vgf_client/vgf_client.py --selftest          # plumbing test, no runner/VKML needed
"""
...
def _rebuild_outputs(work: Path, out_metas: list, user_pos: list | None) -> list:
    """Rebuild the runner's output tensors. vgf_runner.sh wrote each .pte output as raw
...
class VgfExecutor:
    """Runs one `.pte` via the VGF delegate on Vulkan/VKML and returns its outputs. Always
    ...
    def run(self, job_id: str, pte: bytes, inputs: list, timeout: float,
        ...
        with tempfile.TemporaryDirectory(prefix="vgf_job_") as d:
            work = Path(d)
            pte_path = work / "model.pte"
            pte_path.write_bytes(pte)
            cmd = [str(VGF_RUNNER), "--pte", str(pte_path), "--out", str(work),
                   "--target", self.target]
            ...
            try:
                ...
                return P.encode_result(job_id, "TIMEOUT", "VGF runner exceeded job-timeout", None)
            if proc.returncode != 0:
                ...
                return P.encode_result(job_id, status, f"vgf_runner rc={proc.returncode}: {tail}", None)
...
def run_client(host: str, client_port: int, ctrl_port: int, executor: VgfExecutor,
    ...
    print(f"  vgf_client[{label}] → broker tcp://{host}:{client_port} (target={executor.target})",
          flush=True)
    ...
    try:
        while True:
            ...
            ran += 1
            if ran % 50 == 0:
                print(f"  vgf_client[{label}] ran {ran} jobs", flush=True)
    finally:
        executor.close()
        req.close(linger=0)
        sub.close(linger=0)
    print(f"  vgf_client[{label}] done — ran {ran} jobs", flush=True)
    return 0


def main() -> int:
    ap = argparse.ArgumentParser(description="Arm VGF host (Vulkan/VKML emulation) executor client")
    ap.add_argument("--host", default="127.0.0.1")
    ap.add_argument("--client-port", type=int, default=15555)
    ap.add_argument("--ctrl-port", type=int, default=15556)
    ap.add_argument("--label", default="vgf")
    ...
    ap.add_argument("--runner", default=None,
                    help="prebuilt vgf_runner_io binary (else vgf_runner.sh default)")
    ap.add_argument("--selftest", action="store_true",
                    help="echo inputs as outputs (exercise the broker path without the runner)")
    a = ap.parse_args()
    ex = VgfExecutor(a.target, a.runner, selftest=a.selftest)
