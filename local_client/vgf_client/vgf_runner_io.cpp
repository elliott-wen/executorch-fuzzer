/*
 * vgf_runner_io.cpp — host x86 file-I/O runner for the Arm VGF (TOSA→Vulkan) delegate.
 *
 * Runs one ExecuTorch .pte whose graph was partly delegated to the VGF backend. The VGF
 * delegate (vgf_backend, linked whole-archive so it self-registers) JIT-compiles its VGF
 * blob onto a Vulkan >= 1.3 device at method-load time and dispatches compute there; any
 * non-delegated ops fall back to the portable CPU kernels (portable_ops_lib). On this
 * headless host the Vulkan device is Mesa **lavapipe** (CPU software Vulkan) plus the ML
 * SDK **emulation layer** (VK_LAYER_ML_Graph/Tensor_Emulation) — no NPU / GPU needed. The
 * runtime env (VK_INSTANCE_LAYERS / VK_LAYER_PATH / LD_LIBRARY_PATH) is set by vgf_runner.sh.
 *
 * Structurally identical to cadence_runner_io.cpp (same Program/Method file-I/O contract) —
 * the only difference is what is LINKED: a delegate backend + portable kernels, vs. Cadence's
 * custom op library. The delegate is invoked transparently inside method->execute().
 *
 ...
 *   exit 3 = execute() failed (e.g. Vulkan/VGF dispatch error) -> the seam classifies it
 */
...
int main(int argc, char** argv) {
  ...
  // VGF delegate init (Vulkan instance/device + VGF JIT-compile) happens inside load_method.
  ...
  // --- execute (VGF-delegated subgraphs dispatch to Vulkan/VKML here) ---
