#!/usr/bin/env python3
"""Repro: Vulkan operator bug `unsqueeze_copy` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:98 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.436279296875, 0.436279296875, 0.436279296875, -0.11474609375, -0.11474609375, -0.11474609375], dtype=torch.float16).reshape([2, 3, 1])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.unsqueeze_copy.default(B0, 3)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::unsqueeze_copy", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `unsqueeze_copy`  ->", status, detail)
