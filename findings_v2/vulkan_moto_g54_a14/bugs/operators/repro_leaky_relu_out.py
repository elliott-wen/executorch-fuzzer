#!/usr/bin/env python3
"""Repro: Vulkan operator bug `leaky_relu.out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w103:622 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, 1.1103534698486328, 1.1635862588882446, 0.0, 1.724588394165039, 0.0, 0.6749985218048096, 0.0, 0.37696540355682373, 0.0, 0.0, 0.10260883718729019], dtype=torch.float32).reshape([3, 2, 2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.leaky_relu.out(B0, 0.0, out=torch.empty((3, 2, 2), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::leaky_relu_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `leaky_relu.out`  ->", status, detail)
