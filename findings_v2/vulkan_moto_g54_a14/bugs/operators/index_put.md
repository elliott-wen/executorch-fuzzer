...
Bisected from corpus job `w0:15` `out[3]`. The op was rebuilt as a **one-op graph fed its
...
- **worst element** (idx 1): eager **1.0** vs device **0.0**
- input dtypes: ['torch.bool', 'torch.bool']
- sample idx [0, 1] — eager  [1.0, 1.0]
- sample idx [0, 1] — device [1.0, 0.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_index_put.py
```
