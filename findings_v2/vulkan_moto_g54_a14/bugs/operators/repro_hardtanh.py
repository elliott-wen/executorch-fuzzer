#!/usr/bin/env python3
"""Repro: Vulkan operator bug `hardtanh` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w103:644 out[4]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-0.4824913740158081, -0.4824913740158081, 1.1103534698486328, 1.1103534698486328, 0.9340156316757202, 0.9340156316757202, 1.1635862588882446, 1.1635862588882446], dtype=torch.float32).reshape([2, 2, 2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.hardtanh.default(B0, 0, 6)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::hardtanh", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `hardtanh`  ->", status, detail)
