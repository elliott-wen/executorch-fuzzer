...
**Stability:** NON-DETERMINISTIC — wrong output on 2/7 reruns, correct on 5/7 of the SAME baked input (race / uninitialized memory)

## Evidence — single-op isolation with exact baked inputs
Bisected from corpus job `w10:574` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
- **worst element** (idx 0): eager **6.0** vs device **1.0**
- input dtypes: ['torch.int16']
- sample idx [0] — eager  [6.0]
- sample idx [0] — device [1.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_bitwise_and_Scalar_out.py
```
