# Vulkan operator bug — `leaky_relu.out`
...
Bisected from corpus job `w103:622` `out[1]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=1.725e+00 (rtol=0.01 atol=0.001)`
- **worst element** (idx 4): eager **1.72459** vs device **0.0**
- input dtypes: ['torch.float32']
- sample idx [0, 1, 2, 3, 4] — eager  [0.0, 1.11035, 1.16359, 0.0, 1.72459]
- sample idx [0, 1, 2, 3, 4] — device [0.0, 1.11035, 1.16359, 0.0, 0.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_leaky_relu_out.py
```
