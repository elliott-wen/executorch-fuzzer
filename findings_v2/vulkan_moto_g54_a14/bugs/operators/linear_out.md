# Vulkan operator bug — `linear.out`
...
Bisected from corpus job `w106:516` `out[3]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=1.300e+01 (rtol=0.0 atol=0.0)`
- **worst element** (idx 14): eager **6.0** vs device **-7.0**
- input dtypes: ['torch.int64', 'torch.int64']
- sample idx [0, 1, 2, 3, 4, 14] — eager  [-10.0, 6.0, -7.0, -3.0, 6.0, 6.0]
- sample idx [0, 1, 2, 3, 4, 14] — device [-10.0, 6.0, -7.0, -3.0, 6.0, -7.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_linear_out.py
```
