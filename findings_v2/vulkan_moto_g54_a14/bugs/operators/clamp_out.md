...
**Stability:** HIGH — GENUINE on 4/7 re-verification reruns (earlier 3/3 flake was transient device noise)

## Evidence — single-op isolation with exact baked inputs
Bisected from corpus job `w104:379` `out[1]`. The op was rebuilt as a **one-op graph fed its
...
- divergence: `out[0] max|delta|=2.000e+00 (rtol=0.01 atol=0.001)`
- **worst element** (idx 0): eager **-3.0** vs device **-5.0**
- input dtypes: ['torch.int64']
- sample idx [0] — eager  [-3.0]
- sample idx [0] — device [-5.0]

## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_clamp_out.py
```
