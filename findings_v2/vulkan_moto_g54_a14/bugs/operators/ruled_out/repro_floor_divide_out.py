#!/usr/bin/env python3
"""Repro: Vulkan operator bug `floor_divide.out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:751 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.9340156316757202, 1.1635862588882446], dtype=torch.float32).reshape([2])\nB1 = torch.tensor([-inf], dtype=torch.float16).reshape([1])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.floor_divide.out(B0, B1, out=torch.empty((2,), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::floor_divide_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `floor_divide.out`  ->", status, detail)
