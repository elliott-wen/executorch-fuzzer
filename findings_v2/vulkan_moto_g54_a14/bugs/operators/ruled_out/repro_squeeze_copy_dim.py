#!/usr/bin/env python3
"""Repro: Vulkan operator bug `squeeze_copy.dim` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:1069 out[1]),
...
fr = P.encode_pushjob("repro::squeeze_copy_dim", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `squeeze_copy.dim`  ->", status, detail)
