# RULED OUT — `floor_divide.out`

**Status:** not filed as an operator bug. single-op isolation won't lower/run (7/7 device-fail); op was bisected to OPERATOR but is unconfirmable as a one-op graph — needs a 2-op minimal repro.

---

# Vulkan operator bug — `floor_divide.out`
...
Bisected from corpus job `w0:751` `out[1]`. The op was rebuilt as a **one-op graph fed its
...
## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_floor_divide_out.py
```
