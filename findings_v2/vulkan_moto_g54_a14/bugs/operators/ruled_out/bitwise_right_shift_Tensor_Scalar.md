# RULED OUT — `bitwise_right_shift.Tensor_Scalar`

**Status:** not filed as an operator bug. reproduces eager when it runs (2/7 clean) — NOT a kernel bug; the single earlier GENUINE was near-tolerance/transient.
...
Bisected from corpus job `w103:507` `out[2]`. The op was rebuilt as a **one-op graph fed its
...
## Reproduce
```
python findings_v2/vulkan_moto_g54_a14/bugs/operators/repro_bitwise_right_shift_Tensor_Scalar.py
```
