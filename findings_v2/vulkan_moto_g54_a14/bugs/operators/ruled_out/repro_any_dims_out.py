#!/usr/bin/env python3
"""Repro: Vulkan operator bug `any.dims_out` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:389 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([nan, nan], dtype=torch.float16).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.any.dims_out(B0, None, False, out=torch.empty((), dtype=torch.uint8))))\n    return (T,)\n'
