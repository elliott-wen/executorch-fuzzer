#!/usr/bin/env python3
"""Repro: Vulkan operator bug `pixel_shuffle` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w101:415 out[3]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([3.0, 3.0, -3.0, 3.0, 3.0, 3.0, -3.0, 3.0, 3.0, 3.0, -3.0, 3.0, -0.0, -0.0, 4.0, -0.0, -0.0, -0.0, -3.0, 0.0, -2.0, -4.0, -1.0, 0.0, -2.0, -4.0, -4.0, 0.0, -4.0, -2.0, -2.0, 0.0, -3.0, -3.0, 0.0, 0.0, -1.0, -1.0, 2.0, 2.0, 4.0, 4.0, -2.0, -2.0, -1.0, -1.0, 2.0, 2.0, 0.0, 4.0, 2.0, -4.0, 0.0, 3.0, 0.0, -3.0, 0.0, -3.0, -0.0, 3.0, 0.0, -4.0, -2.0, 4.0], dtype=torch.float32).reshape([4, 4, 4])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.pixel_shuffle.default(B0, 2)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::pixel_shuffle", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `pixel_shuffle`  ->", status, detail)
