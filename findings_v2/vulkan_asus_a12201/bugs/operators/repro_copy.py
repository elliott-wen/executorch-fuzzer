#!/usr/bin/env python3
"""Repro: Vulkan operator bug `copy` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:176 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([253, 255, 253, 254], dtype=torch.uint8).reshape([4])\nB1 = torch.tensor([1, 0, 1, 1], dtype=torch.uint8).reshape([4])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.copy.default(B0, B1, False)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::copy", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
