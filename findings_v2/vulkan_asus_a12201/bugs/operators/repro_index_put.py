#!/usr/bin/env python3
"""Repro: Vulkan operator bug `index_put` on Moto G54 5G (Android 14).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:15 out[3]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([True, False], dtype=torch.bool).reshape([2])\nB1 = torch.tensor([True], dtype=torch.bool).reshape([])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.index_put.default(B0, [torch.tensor(1, dtype=torch.int32)], B1, True)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::index_put", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(PL.poll(1000)).get(D):
        ...
        print("op `index_put`  ->", status, detail)
