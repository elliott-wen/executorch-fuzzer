# Vulkan operator bug — `bitwise_xor.Scalar_out`
...
**Stability:** HIGH — GENUINE on 4/7 re-verification reruns

## Evidence — single-op isolation with exact baked inputs
Bisected from corpus job `w0:462` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
## Reproduce
```
python findings_v2/qnn_emulator/bugs/operators/repro_bitwise_xor_Scalar_out.py
```
