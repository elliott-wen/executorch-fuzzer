...
Bisected from corpus job `w0:349` `out[0]`. The op was rebuilt as a **one-op graph fed its
...
- eager  (CPU): [-8.0, -8.0, -8.0, -8.0, -8.0]
- device (GPU): [-7.0, -7.0, -7.0, -7.0, -7.0]

## Reproduce
```
python findings_v2/qnn_emulator/bugs/operators/repro_rsub_Scalar.py
```
