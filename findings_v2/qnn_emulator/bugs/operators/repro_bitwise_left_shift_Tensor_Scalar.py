#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_left_shift.Tensor_Scalar` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:480 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([True, True], dtype=torch.bool).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.bitwise_left_shift.Tensor_Scalar(B0, -7)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::bitwise_left_shift_Tensor_Scalar", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `bitwise_left_shift.Tensor_Scalar`  ->", status, detail)
