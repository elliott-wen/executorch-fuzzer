...
**Stability:** GENUINE on 2/3 reruns  ⚠️ FLAKY / near-tolerance — treat as low-confidence

## Evidence — single-op isolation with exact baked inputs
Bisected from corpus job `w100:382` `out[2]`. The op was rebuilt as a **one-op graph fed its
...
- input dtypes: ['torch.float32', 'torch.uint8']
- eager  (CPU): [1.0, 1.0, 1.0, 1.0]
- device (GPU): [0.0, 0.0, 0.0, 0.0]

## Reproduce
```
python findings_v2/qnn_emulator/bugs/operators/repro_floor_divide.py
```
