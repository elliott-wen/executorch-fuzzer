#!/usr/bin/env python3
"""Repro: Vulkan operator bug `linear` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w127:433 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([27.046875, -7.7734375], dtype=torch.float16).reshape([2])\nB1 = torch.tensor([15.046875, -19.78125], dtype=torch.float16).reshape([2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.linear.default(B0, B1, None)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::linear", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
