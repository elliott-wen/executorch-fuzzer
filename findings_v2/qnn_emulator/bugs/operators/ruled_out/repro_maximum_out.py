#!/usr/bin/env python3
"""Repro: Vulkan operator bug `maximum.out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w102:158 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-inf], dtype=torch.float16).reshape([])\nB1 = torch.tensor([False], dtype=torch.bool).reshape([])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.maximum.out(B0, B1, out=torch.empty((), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::maximum_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `maximum.out`  ->", status, detail)
