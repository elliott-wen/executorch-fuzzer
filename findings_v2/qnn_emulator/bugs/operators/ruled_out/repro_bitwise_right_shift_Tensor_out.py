#!/usr/bin/env python3
"""Repro: Vulkan operator bug `bitwise_right_shift.Tensor_out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w102:767 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([255, 255, 255, 255], dtype=torch.uint8).reshape([2, 1, 1, 2])\nB1 = torch.tensor([0, 0, 0, 0, 0, 0, 0, 0], dtype=torch.int16).reshape([2, 2, 2])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.bitwise_right_shift.Tensor_out(B0, B1, out=torch.empty((2, 2, 2, 2), dtype=torch.int64))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::bitwise_right_shift_Tensor_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `bitwise_right_shift.Tensor_out`  ->", status, detail)
