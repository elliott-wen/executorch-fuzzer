#!/usr/bin/env python3
"""Repro: Vulkan operator bug `logical_not` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w10:420 out[1]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([True, False, False, False, False, True], dtype=torch.bool).reshape([2, 3])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.logical_not.default(B0)))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::logical_not", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `logical_not`  ->", status, detail)
