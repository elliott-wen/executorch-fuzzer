#!/usr/bin/env python3
"""Repro: Vulkan operator bug `exp.out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w102:631 out[3]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([False, True, False, True, True, True], dtype=torch.bool).reshape([3, 1, 2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.exp.out(B0, out=torch.empty((3, 1, 2), dtype=torch.float16))))\n    return (T,)\n'
