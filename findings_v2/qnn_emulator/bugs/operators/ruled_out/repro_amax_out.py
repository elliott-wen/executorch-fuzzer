#!/usr/bin/env python3
"""Repro: Vulkan operator bug `amax.out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:569 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32).reshape([3])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.amax.out(B0, [], True, out=torch.empty((1,), dtype=torch.float32))))\n    return (T,)\n'
