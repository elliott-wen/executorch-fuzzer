#!/usr/bin/env python3
"""Repro: Vulkan operator bug `split_with_sizes_copy` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w0:171 out[2]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([-0.0, 4.44140625], dtype=torch.float16).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.split_with_sizes_copy.default(B0, [1, 0, 0, 1, 0], 0)))\n    return (T,)\n'
