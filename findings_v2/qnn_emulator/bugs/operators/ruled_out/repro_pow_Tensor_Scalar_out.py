#!/usr/bin/env python3
"""Repro: Vulkan operator bug `pow.Tensor_Scalar_out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:160 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, 8.882827758789062], dtype=torch.float32).reshape([2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.pow.Tensor_Scalar_out(B0, 1, out=torch.empty((2,), dtype=torch.float32))))\n    return (T,)\n'
