#!/usr/bin/env python3
"""Repro: Vulkan operator bug `fmod.Scalar_out` on QNN HTP x86 emulator (fp16).
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w100:694 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([4, 3, 1, 4, 0, 1, 3, 4, 1, 3, 2, 4, 1, 0, 4, 3], dtype=torch.uint8).reshape([2, 1, 4, 2])\nLEAVES = [B0]\ndef g(B0):\n    T = _first(_first(torch.ops.aten.fmod.Scalar_out(B0, -7, out=torch.empty((2, 1, 4, 2), dtype=torch.float32))))\n    return (T,)\n'
...
fr = P.encode_pushjob("repro::fmod_Scalar_out", job.pte, job.inputs, job.eager, desc="repro", user_pos=job.user_pos)
...
while time.monotonic() - t0 < 60:
    if dict(P_.poll(1000)).get(D):
        ...
        print("op `fmod.Scalar_out`  ->", status, detail)
