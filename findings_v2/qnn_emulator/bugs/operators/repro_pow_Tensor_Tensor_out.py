...
Single-op graph fed the op's EXACT runtime inputs (baked from corpus job w104:564 out[0]),
...
SRC = 'import torch\ndef _first(r):\n    if isinstance(r, torch.Tensor): return r\n    if isinstance(r, (tuple, list)):\n        for x in r:\n            if isinstance(x, torch.Tensor): return x\n    return r\nB0 = torch.tensor([0.0, 0.0], dtype=torch.float32).reshape([2])\nB1 = torch.tensor([0.0], dtype=torch.float32).reshape([1])\nLEAVES = [B0, B1]\ndef g(B0, B1):\n    T = _first(_first(torch.ops.aten.pow.Tensor_Tensor_out(B0, B1, out=torch.empty((2,), dtype=torch.float32))))\n    return (T,)\n'
